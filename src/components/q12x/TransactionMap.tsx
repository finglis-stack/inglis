...
const TransactionMap = ({ latitude, longitude }: TransactionMapProps) => {
  ...
  if (isError) {
    ...
    return <ErrorDisplay message={error instanceof Error ? error.message : 'Erreur inconnue'} latitude={latitude} longitude={longitude} />;
  }

  if (!apiKey) {
    return <ErrorDisplay message="Clé API Google Maps non disponible" latitude={latitude} longitude={longitude} />;
  }
