...
const EditCardProgram = () => {
  ...
  useEffect(() => {
    const loadProgram = async () => {
      ...
      setFormData({
        ...
        cardType: data.card_type || 'credit',
        currency: data.currency || 'CAD',
        gracePeriod: data.grace_period ? String(data.grace_period) : '21',
        default_interest_rate: data.default_interest_rate ? String(data.default_interest_rate) : '19.99',
        min_interest_rate: data.min_interest_rate ? String(data.min_interest_rate) : '12.99',
        max_interest_rate: data.max_interest_rate ? String(data.max_interest_rate) : '24.99',
        default_cash_advance_rate: data.default_cash_advance_rate ? String(data.default_cash_advance_rate) : '22.99',
        min_cash_advance_rate: data.min_cash_advance_rate ? String(data.min_cash_advance_rate) : '14.99',
        max_cash_advance_rate: data.max_cash_advance_rate ? String(data.max_cash_advance_rate) : '29.99',
        min_income_requirement: data.min_income_requirement ? String(data.min_income_requirement) : '',
        min_credit_score_requirement: data.min_credit_score_requirement ? String(data.min_credit_score_requirement) : '',
        feeModel: data.fee_model || 'none',
        issuanceFee: data.issuance_fee ? String(data.issuance_fee) : '',
        merchantTransactionFee: data.merchant_transaction_fee ? String(data.merchant_transaction_fee) : '',
        binType: data.bin_type || 'shared',
        cardColor: data.card_color || 'linear-gradient(to bottom right, #1e3a8a, #3b82f6)',
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      if (!programId) throw new Error("Identifiant du programme manquant.");
      const updates = {
      ...
      showSuccess('Programme mis à jour avec succès !');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-3 flex items-center justify-between">
        <Link to="/dashboard/settings/card-programs" className="text-sm text-muted-foreground hover:text-primary flex items-center">
      ...
      <Card className="lg:col-span-2">
        <CardHeader>
          ...
          <CardDescription>Mettez à jour les paramètres et le design de votre carte.</CardDescription>
        ...
        <CardContent className="pt-6">
          ...
          {currentStep === 4 && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground">Note: la modification du type de BIN n’affecte pas le BIN existant.</p>
            </div>
          )}
          {currentStep === 5 && (
            <div className="space-y-4">
          ...
          <div className="flex justify-between mt-8">
            ...
            {currentStep < steps.length ? (
              <Button onClick={handleNext}>{t('next', { ns: 'common' })}</Button>
            ) : (
...
export default EditCardProgram;