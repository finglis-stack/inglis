...
serve(async (req) => {
  ...
  try {
    ...
    const vercelApiUrl = `https://api.vercel.com/v9/projects/${VERCEL_PROJECT_NAME}/domains/${domain}/verify${VERCEL_TEAM_ID ? `?teamId=${VERCEL_TEAM_ID}` : ''}`;
    ...
    if (!vercelResponse.ok) {
      throw new Error(vercelData.error?.message || 'Erreur lors de la vérification du domaine.');
    }

    if (vercelData.verified) {
      const supabaseAdmin = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
      await supabaseAdmin.from('custom_domains').update({ status: 'verified', verified_at: new Date().toISOString() }).eq('domain_name', domain);
    }
